...
static inline void float_to_int (float * _f, int16_t * s16, int flags)
{
	...
	break;
    case AC3_CHANNEL | AC3_LFE:
    case AC3_STEREO | AC3_LFE:
	for (i = 0; i < 256; i++) {
...
{
    return oss_open (AC3_STEREO);
}

ao_instance_t * ao_ossdolby_open (void)
{
    return oss_open (AC3_DOLBY);
}

ao_instance_t * ao_oss4_open (void)
{
    return oss_open (AC3_2F2R);
}

ao_instance_t * ao_oss6_open (void)
{
    return oss_open (AC3_3F2R | AC3_LFE);
}
